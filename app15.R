...
ui <- bootstrapPage(
  ...
  absolutePanel(top = 10, right = 10,
                ...
                # 1 # 
                checkboxGroupInput("layers", label = "Select shapefile:", choices = c("Dafor", "Geomorfologia"))      
...
server <- function(input, output, session) {
  ...
  output$map <- renderLeaflet({
    leaflet() %>%
      ...
      if ("Dafor" %in% input$layers) { addPolylines(data = reactiveData()$filtered_dafor,
                                                   fillColor = "#FF0000",
                                                   fillOpacity = 0.5,
                                                   color = "#FF0000",
                                                   weight = 8,
      ...
      if ("Geomorfologia" %in% input$layers){ addPolylines(data = reactiveData()$filtered_geo,
                                                           fillColor = "#0000FF",
                                                           fillOpacity = 0.5,
                                                           color = "#0000FF",
                                                           weight = 8,
